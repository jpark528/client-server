...
void reply_rm(accepted_socket& client_sock, cix_header& header) {
   int length = unlink(header.filename);
   if (length == 0){
      outlog << "ACK: Delete the file" << endl;
      header.command = cix_command::ACK;
      send_packet(client_sock, &header, sizeof header);
      memset (header.filename, 0, FILENAME_SIZE);
   }
   else{
      outlog << "NAK: Fail to remove file." << endl;
      header.command = cix_command::NAK;
      send_packet(client_sock, &header, sizeof header);
   }
...
void reply_put(accepted_socket& client_sock, cix_header& header) {
   ...
   if (outFile) {
      outlog << "ACK: Get file from local side" << endl;
      header.command = cix_command::ACK;
      ...
      outlog << "NAK: Failed to get file from local side" << endl;
      header.command = cix_command::NAK;
      send_packet(client_sock, &header, sizeof header);
   }
   outFile.close();
...
void reply_get(accepted_socket& client_sock, cix_header& header) {
   ...
   if (file) {
      outlog << "Put file on local side" << endl;
   ...
   {
      outlog << "Did not put your file on local side" << endl;
      header.command = cix_command::NAK;
      send_packet (client_sock, &header, sizeof header);
   }
   file.close();
